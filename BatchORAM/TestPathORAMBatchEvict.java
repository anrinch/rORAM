...
import BatchORAM.PathORAMBasicBatchEvict;
...
public class TestPathORAMBatchEvict {


	int N = (int) Math.pow(2,  18); //(int) Math.pow(2, 4);
	int B = 16;
...
/*
	...
	public void testORAMWrites() throws Exception 
	{
		...
		HashMap<Integer, PathORAMBasicBatchEvict.Tree.Block> blockCache = new HashMap<Integer, Tree.Block>();		
		int cacheSize = 8;	
		int rangeSize = 0;
		...
		// generate data
				...
				List<Integer> permutation = new ArrayList<Integer>();
		            for (int j = 0; j < N; j++) { permutation.add(j); }
		            Collections.shuffle(permutation);
		            permutationList.add(permutation);
			
				
		            BitSet[] pm = oram.initialize(si, N, B, data, 0,(int) Math.pow(2, rangeSize),rangeSize, permutationList);
		...
		for (int i = 0; i < iter; i++) {
			...
			if(blockCache.containsKey(k1)) {
				...
				PathORAMBasicBatchEvict.Tree.Block[] b = oram.write(0l, pm, k1, BitSet.valueOf(temp));
				assert (b != null) : "read failed when k = " + k1;
				blockCache.put(b[0].id, b[0]);
			}
			...
			if(blockCache.keySet().size() == cacheSize) {
				...
				oram.rearrangeBlocksAndReturn(0l, blks, cacheSize);
				blockCache.clear();
			}
